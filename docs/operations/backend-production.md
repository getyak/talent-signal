...
The host needs Git, Docker Engine, Docker Compose, the Infisical CLI, public TCP
ports 80 and 443, and enough persistent disk for PostgreSQL and Caddy certificate
state. Point the API hostname at the host before expecting Caddy to obtain a
public certificate.

Store the names listed in `deploy/production/environment.example` under
`prod:/backend` in Infisical. The database password must be URL-safe because
Compose constructs `DATABASE_URL` from the three PostgreSQL fields. `API_DOMAIN`
and the host in `TALENT_SIGNAL_API_BASE_URL` must identify the same endpoint.
Give the host a production-only Machine Identity with read access to
`prod:/shared` and `prod:/backend`; use its short-lived access token as
`INFISICAL_TOKEN`. A human `infisical login` is acceptable only for a supervised
bootstrap inspection, not deployment or unattended operation. The production
wrapper refuses to start without the Machine Identity token.
...
```bash
./scripts/infisical/run.sh prod /shared /backend -- \
...
S3-compatible HTTPS endpoint and path-style addressing in `prod:/backend`.
Prefer the host's AWS workload identity; use
