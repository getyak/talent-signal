...
Today, Sessions, People, and Meetings form one horizontal pager. The header keeps one ordered navigation skeleton while the selected destination expands into one quiet capsule with its symbol and name. The remaining symbols keep their fixed center positions, retain at least 44-point targets, and never disappear; page content does not reflow with the header. Interactive travel may adjust symbol emphasis, while the expanded state changes only when selection changes. Reduced motion applies the selected state directly; full accessibility names, selected traits, and large-content labels remain available. Internal testing belongs in workspace settings when the build or workspace enables it, never as another primary-navigation destination. Page search, filters, scroll and calendar disclosure persist.
