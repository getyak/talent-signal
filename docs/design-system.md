...
Today, Sessions, People, and Meetings form one horizontal pager. The header begins with four equal navigation units. The selected destination expands into one quiet capsule with its symbol and name by borrowing width from one adjacent unit; that pair keeps its combined width, so distant destinations do not move. Unselected symbols retain at least 44-point targets and never disappear; the capsule stays inside the selected control and page content does not reflow with the header. The capsule moves as one continuous focus during a selection change: the outgoing name clears at transition start, then the destination name fades in as the capsule settles. Interactive travel may adjust symbol emphasis, while the expanded state changes only when selection changes. Reduced motion applies the selected state directly; full accessibility names, selected traits, and large-content labels remain available. Internal testing belongs in workspace settings when the build or workspace enables it, never as another primary-navigation destination. Page search, filters, scroll and calendar disclosure persist.
