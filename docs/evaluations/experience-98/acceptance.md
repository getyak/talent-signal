...
| Conversations | Send and canonical reload; stop with queued message; edit pending text; reload paused state; explicit continuation completes once; skip link reaches main; populated Session directory observed at actual 200% Chrome zoom | Deployed post-commit stop/restart repair, prioritization on the current revision, provider failure diagnosis, screen-reader speech and enlarged-text coverage of remaining/error states |
