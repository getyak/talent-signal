...
| --- | --- | --- |
| `pnpm macos:check` | **passed**, exit 0: project generation, companion-trial Node tests, native build, all native unit tests, and UI-test build-for-testing | [`logs/macos-check.log`](logs/macos-check.log) |
| `RUN_MACOS_UI_TESTS=1 pnpm macos:check` | native build/unit/build-for-testing passed, then UI runner did not emit a test event for more than three minutes; run was interrupted and is **not counted as passed** | [`logs/macos-check-ui.log`](logs/macos-check-ui.log) |
| focused `AppModelSafetyTests/testSyntheticDecisionReviewPreservesTheExactSubmittedEvidence` after first fix | **passed**, exit 0 | [`logs/retest-focused-evidence-match.log`](logs/retest-focused-evidence-match.log) |
| retest-v2 native app build | **passed**, exit 0 | [`logs/retest-v2-build.log`](logs/retest-v2-build.log) |
