...
and must complete `screenshot-preprocess.v3` before downstream Agent work. The
...
| Common fields | `screenshot-preprocess.v3` stores platform, conversation kind, participants, messages, identity clues, uncertainty and target-bound follow-up regions; each text region binds the exact unique proper old substring it may replace, never the entire message. Before automatic contact lookup, every identity clue must contain its value in the cited visual excerpt; `contact_name` also requires an equal grounded name clue. Ungrounded provider interpretations remain review-only and cannot become lookup tokens | Zod contract, provider-response and identity-grounding tests |
