...
| Minimize repeat image exposure | Claude receives no original when no follow-up is required; otherwise only flagged source indices are attached, exact regions require pixel receipts, and field-level corrections are host-merged into the preprocessing baseline. The packet and receipts share one aggregate 24-region limit. Plain Chat Completions providers cannot produce those receipts, so they preserve the baseline and abstain without another image disclosure | Claude adapter and correction-boundary unit tests plus PostgreSQL two-image correction and default-provider abstention tests |
| iOS shared path without automatic writes | Manual review and shortcut inbox request `preprocess_only`; the task returns an unconfirmed draft but cannot call contact, capture or research tools. Real HEIC orientation, generic image types, and genuine payloads above the backend limit are normalized, resized, and JPEG-compressed to at most 10 MB for upload while the local original remains unchanged. Each saved fragment retains its preprocessing message ID, source image, visible speaker label/side and visible time without confirming attribution. A later recruiter Retry resumes a failed task at its exact revision; unresolved bounded-read indices remain retryable, and the waiting question and source regions remain visible even when the extraction has no uncertainty text. The preprocessing wire timestamp is truncated to the inbox's persisted second precision so a response-loss retry after app restart retains the identical idempotency request body | iOS client tests, 43 relationship-capture tests, 53 standalone-onboarding tests, backend no-write and retry integration assertions |
...
| Retention and deletion | No second image store; prepared bytes are ephemeral; existing task and canonical-source retention policies remain authoritative. In the foreground review flow, iOS keeps the preprocessing task ID and revision, then deletes the server task before removing its local source on discard or completed filing. Background auto-filing instead proves a shared iOS seed through both operation receipts, checks active relationship and authorization state, and links the task to the exact canonical capture/resource before removing the local draft. Link response loss is idempotently reconciled. Later capture deletion scrubs the linked task and synchronously purges its stored original across the governed descendant resource tree; direct task deletion also purges only that task, independent of global cleanup batch order | PostgreSQL phase-failure/retry, seed/auth linkage, descendant deletion, and target permanent-purge integration tests plus iOS discard/completion/link-recovery tests |
...
Additional local acceptance ran the PostgreSQL-backed integration suite (38/38),
`RelationshipCaptureTests` (43/43), and `StandaloneOnboardingTests` (53/53).
