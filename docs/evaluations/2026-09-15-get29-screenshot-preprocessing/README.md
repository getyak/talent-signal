...
| Common fields | `screenshot-preprocess.v2` stores platform, conversation kind, participants, messages, identity clues, uncertainty and target-bound follow-up regions. Before automatic contact lookup, every identity clue must contain its value in the cited visual excerpt; `contact_name` also requires an equal grounded name, handle, or profile-URL clue. Ungrounded provider interpretations remain review-only and cannot become lookup tokens | Zod contract, provider-response and identity-grounding tests |
