# GET-23 deployment acceptance — September 9, 2026

These are historical acceptance receipts for GET-23 and TestFlight 0.1.70. Later releases may supersede the recorded backend revision, local preview, and release version; use the latest trusted release receipt for the current distribution.

- At the original GET-23 acceptance, the shared TestFlight backend ran `cd402c9897e89cb334656dba184084a3470c9c9c` from the isolated release integration, using image `talent-signal-get23-backend:cd402c98`. The integration preserves deployed account-management commit `90763636` from draft PR #169 and merged GET-25 loading improvements.
...
The unchanged hosted test also passed [three local independent-process diagnostic iterations](hosted-sheet-local-diagnostic.json), in 1.122, 0.932 and 1.044 seconds. This supports a transient hosting/scheduling explanation but does not replace the complete cloud rerun.

## Final completion

Main CI attempt 2 completed successfully on the exact merged commit, including the required aggregate gate. The hosted correction-sheet test passed in 1.737 seconds; all 539 unit tests and all nine isolated UI journeys passed, with zero UI skips. [The final CI readback](main-ci-readback.json) records every required job and its timestamps. No code, timeout, assertion or selection changes were needed for this rerun. The initial failure and independent-process diagnostic receipts remain above.

The deployed API readiness was checked again after CI completion: `status=ready`, `database=ready`, migration `058_product_run_monitor`. [GET-23](https://linear.app/getyak/issue/GET-23) received the final acceptance comment (`43dd32b8`) and was changed from In Progress to **Done**, with the saved status read back in Linear. The exact-version TestFlight receipt and actual Web/iOS/live-Eval evidence satisfy this task's delivery scope; the documented public-login and replay-adapter limitations remain explicit.
