...
Recruiters can attach one or more images to the current relationship Chat
...
- image-only attachments in the desktop relationship Chat composer;
...
- a responsive, keyboard-readable media album bubble for one or many images.
...
- `RelationshipAgentPanel` submits only a 1,000-character objective.
- `useRelationshipAgentController` provides scoped draft recovery and sends a
  UUID-idempotent request through the Next.js integration route.
...
submitted task. A same-origin, authorized backend route streams bounded content
through the configured local or S3-compatible adapter and marks the asset ready
only after storage succeeds. Authorized reads use the same scoped proxy.
...
4. Verify contracts, lifecycle, scope isolation, idempotency, UI semantics,
   light/dark/reduced-motion/mobile behavior, and production build.
...
- The 390 by 844 viewport had no horizontal overflow and kept a 12-pixel gap
  between the composer and mobile dock; light and dark surfaces were inspected.
...
- `pnpm docs:check`, backend and web tests, typechecks, web lint, backend and web
  production builds, both Compose configuration checks, and `git diff --check`
