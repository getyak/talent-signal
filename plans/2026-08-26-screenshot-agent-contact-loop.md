...
- allowlisted Anthropic/OpenRouter Agent environment import into ignored mode
...
Generalize the existing credential-sync utility to choose Claude when an
Anthropic key and pinned model are present, retain OpenRouter as an explicit
alternative, preserve unrelated local target settings, and copy only provider
credentials needed by the Agent. The supplied unrelated database and service
