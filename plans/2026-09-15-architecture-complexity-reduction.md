...
Milestone 3 replaced seven independent `RelationshipAskView` operation fields with one `RelationshipAskOperationState`, centralized operation replacement/cancellation, assigned every screenshot, person-research, unscoped, and scoped operation a non-optional ownership token, and rejected stale finish/cancel callbacks before they can release newer UI state. It also consolidated repeated return-to-idle transitions without changing evidence, retry, or authorization behavior. On the allowlisted iPhone 17 Pro Simulator, 5 focused state tests, 574 non-Keychain unit tests, and 3 focused Relationship Ask UI tests passed; the Release Simulator build, documentation checks, architecture diagrams, and localization checks also passed. The same six unsigned-host Keychain tests remained excluded for signed CI verification.
