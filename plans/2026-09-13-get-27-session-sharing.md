...
- The default card contains the minimum useful context; sharing a bounded readable transcript remains an explicit secondary choice.
...
1. Introduce a pure Session share policy that produces a minimal card payload and a separately selected bounded-conversation snapshot.
...
- Screenshots show the preview, scope control, safety boundary, and final system-share control from the latest commit.
