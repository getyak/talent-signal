...
- Pull requests 57–60 added a main-only signing-maintenance path, then hardened
  its regenerated-profile import with exact bundle, name, entitlement, and
  embedded-certificate verification. An authorized maintainer enabled Sign in
  with Apple on the App ID and regenerated the existing App Store profile.
  Maintenance run
  [32813906656](https://github.com/getyak/talent-signal/actions/runs/32813906656)
  encrypted that verified profile into the isolated match repository at commit
  `62682c6f5c9d961fecedaf267373b921de62f490`. The temporary write deploy key and
  environment secret were then revoked and removed; ordinary release access
  remains read-only.
- Rerunning the failed job of the original automatic `workflow_run` release
  [32802481210](https://github.com/getyak/talent-signal/actions/runs/32802481210)
  preserved its successful-main provenance and completed the entire publish
  path. App Store Connect processed version 0.1.6 build `20260825054605`; the
  workflow created tag `v0.1.6`, the matching
  [GitHub prerelease](https://github.com/getyak/talent-signal/releases/tag/v0.1.6),
  provenance attestation, retained IPA, and Actions artifact.
- A separate read-only App Store Connect audit
  [32814745604](https://github.com/getyak/talent-signal/actions/runs/32814745604)
  found the tester `INSTALLED` on one known device, an active account with no
  pending invitation, membership in the all-build internal group, and version
  0.1.6 build `20260825054605` in `VALID` state. It made no relationship or
  invitation changes and concluded `SERVER_ACCESS_READY`.
...
5. **Complete — Real delivery:** merging the pinned signing configuration to
   ...
   retained IPA, and artifact `TalentSignal-0.1.5`. The later 0.1.6 delivery and
   read-only access audit supersede the earlier invited-device unknown: App
   Store Connect now reports one installed device with valid access to the
   latest build.
...
7. **Complete — Release-gate recovery:** external HTTP reachability is advisory,
   repository-owned documentation integrity remains required, and CI and
   release use one iOS change classifier. Main CI run 32800735453 passed and
   started the release that ultimately processed 0.1.6 in TestFlight.
8. **Complete — Signing-profile repair:** the explicit main-only maintenance
   workflow imported the maintainer-regenerated Sign in with Apple profile only
   after checking its identity, entitlement, and certificate. The short-lived
   write credential was removed, and the original automatic release completed
   App Store Connect processing with ordinary read-only match access.
...
Complete. A successful `main` CI run automatically triggered the release chain,
and its failed publish job resumed after the scoped signing repair without
changing the source revision or bypassing the CI gate. TestFlight processed
version 0.1.6 build `20260825054605`; tag, prerelease, attestation, IPA, and
artifact evidence exist. A separate non-mutating App Store Connect audit reports
the build valid and accessible through the all-build internal group, with an
active tester in `INSTALLED` state on one known device. The temporary signing
maintenance credential has been removed, so subsequent automatic releases use
the isolated read-only signing path.
