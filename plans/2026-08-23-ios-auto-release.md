...
8. **Active — Signing-profile repair:** use an explicit main-only maintenance
   workflow and a short-lived write deploy key to synchronize the App Store
   profile regenerated by an authorized maintainer. The first automation run
   proved the App Manager API key can read but cannot create profiles. Apple
   Developer now has an active regenerated profile containing Sign in with
   Apple; the remaining work is to encrypt it into match, revoke the key, and
   rerun the already verified automatic release. Completion requires App Store
   Connect processing, not merely a successful match commit.
