...
async function storeChat(client:PoolClient,auth:AuthContext,row:Row,displayName?:string) {
  ...
  if(extraction.conversation_kind!=="direct"&&!(extraction.conversation_kind==="not_chat"&&extraction.contact_name&&extraction.identity_clues.some(clue=>["profile_url","handle","company","job_title"].includes(clue.kind)))&&!row.state.selected&&!row.state.user_contact_label)deny("CONTACT_CHAT_IDENTITY_AMBIGUOUS");
  ...
  const request:ResourceCaptureRequest={contract_version:CONTRACT_VERSION,idempotency_key:clientResourceID,
    channel:manifest.source?"browser_extension":extraction.conversation_kind==="not_chat"?"web_upload":"chat",purpose:"User-authorized internal person filing from an intentional capture; extracted content remains proposed",
