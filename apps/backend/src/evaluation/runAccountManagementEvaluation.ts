...
try{
  ...
  const explicitSessionId=(await pool.query<{id:string}>('SELECT id FROM sessions WHERE account_id=$1 AND user_id=$2 ORDER BY created_at DESC LIMIT 1',[owner.account.id,memberId])).rows[0]!.id;
