...
const databaseURL = process.env.ANSWER_FEEDBACK_UI_DATABASE_URL;
...
assert(parsedDatabase && parsedDatabase.protocol === "postgresql:" && !parsedDatabase.search && !parsedDatabase.hash
  ...
  && parsedDatabase.pathname === "/get11_proof" && process.env.ANSWER_FEEDBACK_UI_DISPOSABLE === "true",
"Use the explicitly owned disposable get11_proof database.");
const port = 4341;
process.env.TALENT_SIGNAL_BACKEND_REVISION = "answer-feedback-native-proof-v1";
...
app.get("/proof-state", async () => ({ purpose: "owned-native-answer-feedback-proof", real_provider: false,
...
app.post("/proof-prepare", async (_request, reply) => {
  ...
  const fixture = { purpose: "owned-native-answer-feedback-proof", real_provider: false, backend_url: `http://127.0.0.1:${port}`,
...
console.log(JSON.stringify({ purpose: "owned-native-answer-feedback-proof", listening: port, real_provider: false }));
