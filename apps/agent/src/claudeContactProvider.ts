...
export class ClaudeContactAgentModel implements ContactAgentModel {
  ...
  async run(input: Parameters<NonNullable<ContactAgentModel["run"]>>[0], signal: AbortSignal) {
    ...
    const views = await screenshotImageViews(input.images, input.readImage, signal);
    ...
    const result = await this.execute(this.configuration, {
      ...
      tools,
      subagents: [{ name: "screenshot-source-review", description: "Independently verify a bounded ambiguous original-image region, speaker attribution or exact quotation when a separate source check will help. Do not delegate simple readable captures.",
