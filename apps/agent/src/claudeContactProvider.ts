...
export class ClaudeContactAgentModel implements ContactAgentModel {
  ...
  async run(input: Parameters<NonNullable<ContactAgentModel["run"]>>[0], signal: AbortSignal) {
    ...
    const sourceReview = views ? screenshotSourceReview(views, input.preprocessing?.sources.flatMap(source =>
      source.follow_up_regions.map(item => ({source_image_index:source.source_image_index,field:item.field,region:item.region}))) ?? [],
    ...
    const result = await this.execute(this.configuration, {
      ...
      tools,
      subagents: input.text || !sourceReview ? [] : [{ name: "screenshot-source-review", description: "Optional independent source check. Agent prompt MUST be only JSON: {source_image_index:0,region:{left:0,top:0,width:700,height:1400},field:'text'}. Use JSON double quotes. field is text, speaker, time or identity. No expected answer, guess, prose, prior conversation or extra keys. Do not delegate simple readable captures.",
