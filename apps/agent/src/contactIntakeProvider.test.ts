...
describe("contact Agent provider",()=>{
  it("grounds web text extraction in exact source substrings and uses supported reasoning",async()=>{
    ...
    const result=await provider(fetcher).extractText(raw,signal());
    expect(result.extraction.messages[0]).toMatchObject({message_id:"m1",sequence:0,speaker_side:"unknown"});
    const body=JSON.parse(String(fetcher.mock.calls[0]?.[1]?.body));expect(body.thinking.type).toBe("enabled");expect(body.reasoning_effort).toBe("low");
    const bad=vi.fn<typeof fetch>().mockResolvedValue(new Response(JSON.stringify({id:"text-receipt",model:"fixture-tools",choices:[{message:{content:JSON.stringify({...extraction,messages:[{...extraction.messages[0],text:"Invented source"}]})}}]})));
    await expect(provider(bad).extractText(raw,signal())).rejects.toThrow("CONTACT_TEXT_EXTRACTION_NOT_SOURCE_GROUNDED");
  });
  ...
  it("keeps profile paragraphs as exact source blocks without fabricating chat",async()=>{
    const raw="Example Person\nExample Labs · Designer";
    const fetcher=vi.fn<typeof fetch>().mockResolvedValue(new Response(JSON.stringify({id:"text-receipt",model:"fixture-tools",choices:[{message:{content:JSON.stringify({...extraction,conversation_kind:"not_chat",messages:[]})}}]})));
    const result=await provider(fetcher).extractText(raw,signal());
    expect(result.extraction.messages).toEqual([{message_id:"m1",sequence:0,text:raw,speaker_side:"unknown",speaker_label:null,time_text:null}]);
  });
