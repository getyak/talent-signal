...
export function calendarDraftCapability(context: CalendarDraftContext | undefined, objective: string) {
  ...
  const tool: HarnessTool = {
    ...
    description: `Prepare one editable calendar draft for the user's current request, without saving an event, inviting anyone, or scheduling a reminder. Reference instant: ${context.referenceTime}; user's timezone: ${context.timeZone}; reference local date: ${new Intl.DateTimeFormat("en-CA", { timeZone: context.timeZone, year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date(context.referenceTime))}. This reference exclusively defines today for this request, even if SDK environment dates or prior messages differ. Resolve relative dates using that reference, include the explicit UTC offset in both local timestamps, preserve stated duration, and quote the exact supporting user text. If a date or duration is materially unclear, ask before staging. A successful draft is the end of preparation and still requires an explicit human calendar action.`,
    execute: async raw => {
