...
export const COMPOSER_MENTION_DISCLOSURE =
  "仅插入姓名与背景文字。查看这位联系人的私密资料，请打开人物页。";
...
export function detectComposerTrigger(
  ...
  for (let index = cursor - 1; index >= 0; index -= 1) {
    ...
    while (end < value.length) {
      if (TOKEN_CHAR.test(value[end]!)) { end += 1; continue; }
      if (char === "@" && value[end] === "." && end + 1 < value.length && TOKEN_CHAR.test(value[end + 1]!)) {
        end += 1; continue;
      }
      break;
    }
...
export function composerLengthState(
  ...
  const rawLength = Math.max(0, typeof value === "string" ? value.length : value);
  const safeLength = typeof value === "string" ? value.trim().length : rawLength;
  const remainingToSend = Math.max(0, sendLimit - safeLength);
  if (rawLength >= draftLimit) {
    return {
