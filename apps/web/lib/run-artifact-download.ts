...
export async function loadRunArtifactBlob(taskID:string,file:RunArtifact,sessionVersion:string,signal:AbortSignal,request:typeof fetch=fetch){
  ...
  const response=await request(`${path}/${file.id}`,{cache:"no-store",headers,signal});signal.throwIfAborted();
  ...
  const current=await request(path,{cache:"no-store",headers,signal});signal.throwIfAborted();
