...
export function readHomeConversationDraft(storageScope: string): PendingSessionDraft | null {
  try {
    const prefix = `talent-signal:session-draft-pending:v1:${storageScope}:`;
    let latest: PendingSessionDraft | null = null;
    for (let i = 0; i < window.localStorage.length; i += 1) {
      const key = window.localStorage.key(i);
      if (!key?.startsWith(prefix)) continue;
      const pending = readPendingSessionDraft(storageScope, key.slice(prefix.length));
      if (!pending || (pending as PendingSessionDraft & {purpose?: string}).purpose !== "conversation-home") continue;
      if (!latest || pending.savedAt > latest.savedAt) latest = pending;
    }
    return latest;
  } catch { return null; }
}
