...
import { completeWebTelemetryTrace } from "@/lib/server/telemetryBackend";
...
export async function POST(
  ...
  if (!TRACE_ID.test(traceId) || !body || !matchesTypeBox(CompleteTelemetryTraceRequestSchema, body)) {
    return NextResponse.json({ code: "telemetry_completion_invalid" }, { status: 400 });
  }
  try {
    return NextResponse.json(await completeWebTelemetryTrace(traceId, body));
  } catch (error) {
