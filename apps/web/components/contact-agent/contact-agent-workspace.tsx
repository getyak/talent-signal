...
export function ContactAgentWorkspace({personID,contextID,initialTaskID}:{personID?:string;contextID?:string;initialTaskID?:string}){
  ...
  useEffect(()=>{
    ...
    let current=true;request<Task>(`tasks/${initialTaskID}`).then(value=>{if(current)setTask(value);}).catch(e=>{if(current)setError(e.message);});
  ...
  return <div className={styles.page}>
    ...
    <main className={styles.layout} id="main-content">
      <aside className={styles.sidebar}><p className={styles.eyebrow}>{personID?"这位联系人的整理记录":"最近整理"}</p>{scopedRecent.length===0?<p className={styles.muted}>你的截图任务会保留在这里。</p>:scopedRecent.map(item=><button key={item.task_id} onClick={()=>void openTask(item.task_id)} className={item.task_id===taskID?styles.selected:""}><strong>{item.contact?.display_name??"截图"}</strong><span>{states[item.status]} · {new Date(item.created_at).toLocaleDateString("zh-CN")}</span></button>)}</aside>
      <div className={styles.content}>
        <div className={styles.intro}><p className={styles.eyebrow}>{personID?"联系人档案":"截图 → 联系人"}</p><h1>{personID?personName??"联系人":"从截图整理联系人与对话。"}</h1><p>聊天保存为带来源的记录，个人主页整理为可核对的资料草稿。</p>
          {personID&&revision&&!archiveID&&<button className={styles.textButton} onClick={()=>setArchiveOpen(true)}>归档联系人</button>}
        </div>
        {archiveID?<div className={styles.card}><p>联系人已归档，资料已从当前工作区隐藏。</p><button onClick={()=>void restore()}>撤销归档</button></div>:<details open={!personID} className={personID?styles.profileComposer:undefined}><summary hidden={!personID}>追加截图</summary><section className={styles.composer} aria-label="导入截图">
          <label className={styles.upload}><span>＋ {attachments.length?"添加图片":"选择截图"}</span>{attachments.length>0&&<span>{attachments.length} / 10</span>}<input type="file" multiple accept="image/png,image/jpeg,image/webp" aria-label="选择截图" onChange={event=>{selectFiles(event.target.files);event.target.value="";}} disabled={busy||status==="running"}/></label>
        ...
        {shown.map(item=><section className={styles.card} key={item.task_id} id={`contact-task-${item.task_id}`} aria-label="联系人分析卡片">
          ...
          {Boolean(item.extraction?.messages.length)&&item.extraction&&<details className={styles.section}><summary>聊天原文 · {item.extraction.messages.length} 条</summary><ol className={styles.messages}>{item.extraction.messages.map(message=><li key={message.message_id}><small>{item.source_images?.[message.source_image_index??-1]&&<><a href={`/api/contact-agent/tasks/${item.task_id}/images/${message.source_image_index}`} target="_blank" rel="noreferrer">图片 {(message.source_image_index??0)+1}</a> · </>}{message.message_id} · {message.speaker_side==="left"?"画面左侧":message.speaker_side==="right"?"画面右侧":"说话人未确定"}{message.time_text?` · ${message.time_text}`:""}</small><p>{message.text}</p></li>)}</ol></details>}
