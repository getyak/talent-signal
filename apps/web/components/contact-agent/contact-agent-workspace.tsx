...
export function ContactAgentWorkspace({personID,contextID,embedded=false,initialTaskID}:{personID?:string;contextID?:string;embedded?:boolean;initialTaskID?:string}){
  ...
  async function submit(){
    ...
    try{
      if(inputMode==="text"&&!attempt.current)attempt.current={idempotency_key:crypto.randomUUID(),objective:objective.trim()||"整理这段来源，识别人物并保存准确证据。身份不清时询问，没有人物不创建。",text:text.trim(),source:{kind:"selected_text",title:"粘贴的文字",url:"",time_basis:"imported_at"},allow_public_research:research,captured_at:new Date().toISOString()};
    ...
    <main className={styles.layout} id="main-content">
      ...
      <div className={styles.content}>
        ...
        {shown.map(item=><section className={styles.card} key={item.task_id} id={`contact-task-${item.task_id}`} aria-label="联系人分析卡片">
          <div className={styles.cardHeading}><div><p className={styles.eyebrow} role="status">{states[item.status]}</p><h2>{item.contact?.display_name??item.source?.title??(item.status==="completed"?"已检查来源":"采集结果")}</h2></div>{item.contact&&<Link className={styles.profileLink} href={`/workspace/captures/people/${item.contact.person_id}?context=${item.contact.relationship_context_id}`}>打开档案 ↗</Link>}</div>
