...
describe("combined remote and local send races", () => {
  it.each([true, false])("retains competing content and blocks every save before rebase (identity=%s)", (hasIdentity) => {
    const competing = pendingDraft({ idempotencyKey: otherKey, draft: "B pending unsaved", updatedAt: newerAt });
    const completion = resolveSessionSendCompletion({
      competing,
      identity: hasIdentity ? { composerDraft: "A sent", composerDraftUpdatedAt: persistedAt } : undefined,
      readback: detail({ composer_draft: "C new server draft", composer_draft_updated_at: newerAt, revision: 17 }),
      storage: "competing",
    });
    expect(completion.state.draft).toBe("B pending unsaved");
    expect(completion.state.lastSavedDraft).toBe("C new server draft");
    expect(completion.state.detail.revision).toBe(17);
    expect(completion.pending).toBe(competing);
    expect(completion.foreign).toBe(true);
    expect(completion.cleanupWrite).toBe(false);
    expect(completion.state.conflict).toBe(true);
    // executePersist and its button use this gate BEFORE any pending rebase.
    expect(canPersistSessionDraft(completion.state)).toBe(false);
    expect(buildSaveRequest(completion.state, sendKey, newerAt)).toBeNull();
    const edited = applyDraftInput(completion.state, "B pending with explicit edit");
    expect(canPersistSessionDraft(edited)).toBe(false);
  });

  it("does not resurrect a competing draft after the server deletes the session", () => {
    const competing = pendingDraft({ idempotencyKey: otherKey, draft: "retained content", updatedAt: newerAt });
    const completion = resolveSessionSendCompletion({
      competing, identity: undefined, storage: "competing",
      readback: detail({ state: "deleted", composer_draft: null, composer_draft_updated_at: null }),
    });
    expect(completion.pending).toBeNull();
    expect(completion.state.draft).toBe("");
    expect(canPersistSessionDraft(completion.state)).toBe(false);
