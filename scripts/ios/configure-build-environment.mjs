...
export function validateWebOrigin(value, configuration) {
  if (!value?.trim()) return "";
  let normalized;
  try {
    normalized = validateAPIBaseURL(value.trim(), configuration);
  } catch (error) {
    throw new Error(error.message.replaceAll("TALENT_SIGNAL_API_BASE_URL", "TALENT_SIGNAL_WEB_ORIGIN"));
  }
  const url = new URL(normalized);
  if (url.pathname !== "/") {
    throw new Error("TALENT_SIGNAL_WEB_ORIGIN must be an origin without a path.");
  }
  return url.origin;
}
...
export function writeBuildEnvironment({
  ...
  const webOriginSource = environment.TALENT_SIGNAL_WEB_ORIGIN
    ?? (existsSync(environmentFile ?? defaultEnvironmentFile)
      ? readEnvironmentValue(readFileSync(environmentFile ?? defaultEnvironmentFile, "utf8"), "TALENT_SIGNAL_WEB_ORIGIN") : undefined);
  const webOrigin = validateWebOrigin(webOriginSource, configuration);
  const contents = [
    ...
    `TALENT_SIGNAL_WEB_ORIGIN_BASE64URL = ${encodeAPIBaseURL(webOrigin)}`,
    "",
  ].join("\n");
