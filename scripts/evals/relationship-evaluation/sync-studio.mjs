...
const semantic=await readFile(path.join(root,'runs',label,'semantic-review.json'),'utf8').then(JSON.parse).catch(error=>{if(error.code==='ENOENT')return null;throw error;});
const runBytes=new Map(await Promise.all(suite.cases.map(async c=>[c.id,await readFile(path.join(root,'runs',label,c.id+'.json')).catch(error=>{if(error.code==='ENOENT')return null;throw error;})])));
if(semantic&&(semantic.label!==label||semantic.complete!==true||semantic.humanGold!==false))throw Error('Semantic review provenance incomplete');
if(semantic)for(const c of suite.cases){
  const reviewed=semantic.cases.find(row=>row.caseID===c.id);
  if(!reviewed||!runBytes.get(c.id)||!['pass','fail','unknown'].includes(reviewed.status)||reviewed.sourceSha256!==hash(runBytes.get(c.id)))
    throw Error('Semantic review source mismatch '+c.id);
}
const rows=[];
...
for(const c of suite.cases) {
  ...
  const run=runBytes.get(c.id)?JSON.parse(runBytes.get(c.id).toString('utf8')):null;
  ...
  const value={...saved,expected:JSON.stringify(displayedExpectations,null,2),actual,observed:run?(verdict.status==='fail'||semanticReview?.status==='fail'?'fail':'unknown'):'unrun',decision:'pending'};
  ...
  rows.push({id:c.id,title:c.title,category:c.category,verdict,semanticReview,observed:value.observed,caseID:receipt.caseID,reply:run?.result?.block?.body??'',error:run?.error?.message??'',expected:displayedExpectations,actual});
}
const readback=await fetch(origin+'/api/projects').then(r=>r.json());
for(const row of rows) {
  const actual=readback.cases.find(c=>c.id===row.caseID);
  if(!actual||actual.actual!==row.actual||actual.decision!=='pending'||actual.observed!==row.observed)throw Error('Result readback failed '+row.id);
}
...
const cards=rows.map(r=>`<article data-mechanical="${r.verdict.status}" data-status="${r.semanticReview?.status==='fail'?'fail':r.verdict.status}"><div><span class="tag">${escape(r.id)} · ${escape(r.verdict.status)} · AI语义 ${escape(r.semanticReview?.status??'未复核')}</span><h2>${escape(r.title)}</h2><p class="fail">${escape(r.verdict.failures.join(' · ')||r.error)}</p><details><summary>预期行为（待人工校准）</summary><pre>${escape(JSON.stringify(r.expected,null,2))}</pre></details>${r.semanticReview?`<details><summary>独立语义复核（非人工标注）</summary><pre>${escape(JSON.stringify({status:r.semanticReview.status,observations:r.semanticReview.observations,findings:r.semanticReview.findings},null,2))}</pre></details>`:''}<h3>实际回复</h3><p class="reply">${escape(r.reply||r.error||'尚未运行')}</p><a href="${escape(origin)}/#project/${generation.studioProject.id}/case/${r.caseID}">打开案例与原始记录 →</a></div><img loading="lazy" src="png/${r.id}.png" alt="${escape(r.id)} 合成聊天截图"></article>`).join('');
await writeFile(path.join(preview,'index.html'),`<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>关系对话评测 · ${escape(label)}</title><style>body{font:16px/1.65 system-ui;margin:0;background:#f5f4ef;color:#26352f}main{max-width:1120px;margin:auto;padding:40px 24px}h1{font-size:36px;margin:8px 0}h2{font-size:21px;margin:12px 0}.intro{max-width:800px;color:#546158}.tag{font:13px ui-monospace;background:#e7e8df;padding:5px 9px;border-radius:5px}article{display:grid;grid-template-columns:1fr 280px;gap:40px;border-top:1px solid #d5d8cd;padding:36px 0}article[hidden]{display:none}article img{width:100%;border:1px solid #d8d9d1;border-radius:20px}pre{white-space:pre-wrap;font-size:13px}.reply{white-space:pre-wrap}.fail{color:#923e31}a{color:#246551}details{margin:18px 0}h3{font-size:14px;letter-spacing:.08em;color:#627468}nav{position:sticky;top:0;background:#f5f4eff0;padding:12px 0}button{border:1px solid #b8c6ba;border-radius:16px;padding:7px 14px;background:white;margin:4px;cursor:pointer}@media(max-width:650px){article{grid-template-columns:1fr;gap:20px}article img{max-width:330px}main{padding:20px}h1{font-size:28px}}</style><main><p>Talent Signal / Evaluation</p><h1>关系对话，逐条验证。</h1><p class="intro">${escape(label)} · 24 个合成聊天案例。图片、预期、真实工具记录与回复分别保留。机械检查通过不代表人工质量认证；未运行、失败和基础设施错误均不会显示为通过。</p><nav>${['all','fail','error','mechanical_pass','unrun'].map(s=>`<button onclick="document.querySelectorAll('article').forEach(a=>a.hidden='${s}'!=='all'&amp;&amp;('${s}'==='mechanical_pass'?a.dataset.mechanical:a.dataset.status)!=='${s}')">${s}</button>`).join('')}</nav>${cards}</main></html>`);
