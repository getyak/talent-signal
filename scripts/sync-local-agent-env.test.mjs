...
test("copies only allowlisted Claude settings and preserves local target values", async (context) => {
  ...
  await writeFile(
    ...
    [
      ...
      "TALENT_SIGNAL_AGENT_MODEL=claude-synthetic-pinned",
  ...
  assert.equal(values.ANTHROPIC_API_KEY, "synthetic-secret-value");
...
test("fails without an Anthropic key and does not create the target", async (context) => {
  const directory = await mkdtemp(join(tmpdir(), "talent-signal-agent-env-"));
  context.after(() => rm(directory, { force: true, recursive: true }));
  const source = join(directory, "source.env");
  const target = join(directory, "target.env");
